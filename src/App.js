...
class App extends Component {
  ...
  componentDidMount() {
    ...
    if (window.localStorage.getItem("cUser")) {
      ...
      window
        .fetch(`${SERVER}/api/v1/users/${userID}`)
        .then((resp) => resp.json())
        .then((json) => {
          if (json.id === userID) {
            this.setState((prevState) => ({
              current_user: JSON.parse(window.localStorage.getItem("cUser")),
              current_user_roles: JSON.parse(
                window.localStorage.getItem("cUserRoles")
              ),
              current_user_teams: JSON.parse(
                window.localStorage.getItem("cUserTeams")
              ),
              loggedIn: true,
            }));
          } else {
  ...
  findMembersOfTeam = (teamID) =>
    this.findPositionsOnTeam(teamID).filter((p) => this.findUserByID(p.id));

  findUserPositions = (userID) =>
  ...
  updatePositionsUser = (positionID, userID) => {
    window
      .fetch(`${SERVER}/api/v1/positions/${positionID}`, {
  ...
  findUserTeams = (user_id) => {
    ...
    const positionTeams = positions.map((position) =>
      this.getTeamById(position.team_id)
    );
    const adminTeams = this.state.teams.filter(
      (team) => team.admin === user_id
    );
  ...
  getTeamById = (team_id) =>
    this.state.teams.find((team) => team.id === team_id);
  ...
  addUserRoleToState = (userRole) => {
    ...
    window.localStorage.setItem(
      "cUserRoles",
      JSON.stringify(this.state.current_user_roles)
    );
  };
  ...
  addTeamToState = (Team) => {
    ...
    window.localStorage.setItem(
      "cUserTeams",
      JSON.stringify(this.state.current_user_teams)
    );
  };
  ...
  changeUserName = (string) => {
    if (this.state.current_user) {
      ...
      window.localStorage.setItem(
        "cUser",
        JSON.stringify(this.state.current_user)
      );
    } else {
    }
  };
  ...
  destroyUserRole = (ur) => {
    return window
      .fetch(`${SERVER}/api/v1/user_roles/${ur.id}`, {
        headers: { "Content-Type": "application/json; charset=utf-8" },
        method: "DESTROY",
      })
      .then((res) => {
  ...
  setChangeOwnerTeam = (team, user) => {
    ...
    return window
      .fetch(`${SERVER}/api/v1/teams/${team.id}`, {
        headers: { "Content-Type": "application/json; charset=utf-8" },
        body: JSON.stringify({
          admin: user.id,
        }),
  ...
  removeTeamFromState = (team) => {
    ...
    if (index2 !== -1) {
      ...
      localStorage.setItem(
        "cUserTeams",
        JSON.stringify(current_user_teams_array)
      );
    }
